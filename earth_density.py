...
if __name__ == "__main__":
    for n in np.linspace(0, 0.9, 9):
        X = x(theta := np.pi * n)
        Z = np.linspace(0, X)
        d = [_d_earth(z, theta) for z in Z]
        plt.plot(Z, d, label=f"$\\theta = {n:.1f}\pi$")
...
def attenuation_integral(theta):
    """Returns the integral of d_earth described above."""
    return quad( _d_earth, 0, x(theta), args=[theta] )[0]
...
if __name__ == "__main__":
    ...
    plt.xlabel("$\\theta$ (rad)")
    plt.ylabel("$\\alpha(\\theta) := \int_0^{x(\\theta)}{d(r(z, \\theta)) \\: d z}$ ($g/m^2$)")
